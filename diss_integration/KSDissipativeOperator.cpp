...
capd::interval KSDissipativeOperator<m, M>::D1ksmall(const capd::pdes::PolynomialBound<capd::interval, unsigned, M> &pb) const{
    capd::interval res = 0;
    for(int k = M; k < 2 * M; ++k){
...
template <int m, int M>
capd::interval KSDissipativeOperator<m, M>::D1klarge(const capd::pdes::PolynomialBound<capd::interval, unsigned, M> &pb) const{
    capd::interval res = 0;
    auto s = pb.exponent();
    auto C = pb.C();
    for(int n = 0; n < M; ++n){
...
template <int m, int M>
capd::interval KSDissipativeOperator<m, M>::D1(const capd::pdes::PolynomialBound<capd::interval, unsigned, M> &pb) const{
    return capd::max(KSDissipativeOperator<m, M>::D1ksmall(pb), KSDissipativeOperator<m, M>::D1klarge(pb)).rightBound();
}


template <int m, int M>
capd::interval KSDissipativeOperator<m, M>::FS_minus_galerkin(int k, const capd::pdes::PolynomialBound<capd::interval, unsigned, M> &pb) const{
    capd::interval res = 0;
    auto s = pb.exponent();
    auto C = pb.C();
...
template <int m, int M>
capd::interval KSDissipativeOperator<m, M>::FS(int k, const capd::pdes::PolynomialBound<capd::interval, unsigned, M> &pb) const{
    capd::interval res = 0;
    auto s = pb.exponent();
    auto C = pb.C();
    ...
    return res + FS_minus_galerkin(k, pb);
}


// D_2 =  \frac{C}{M + 1} (\frac{C}{(M + 1)^{s - 1} (s - 1)} + \sum_{n = 0}^{M - 1} \abs{a_n})
template <int m, int M>
capd::interval KSDissipativeOperator<m, M>::D2(const capd::pdes::PolynomialBound<capd::interval, unsigned, M> &pb) const{
    capd::interval res = 0;
    auto s = pb.exponent();
    auto C = pb.C();
...
template <int m, int M>
capd::interval KSDissipativeOperator<m, M>::IS_minus_galerkin(int k, const capd::pdes::PolynomialBound<capd::interval, unsigned, M> &pb)  const{
    capd::interval res = 0;
    auto s = pb.exponent();
    auto C = pb.C();
...
template <int m, int M>
capd::interval KSDissipativeOperator<m, M>::IS(int k, const capd::pdes::PolynomialBound<capd::interval, unsigned, M> &pb)  const{
    capd::interval res = 0;
    auto s = pb.exponent();
    auto C = pb.C();
    // IS(k) \subset \sum_{n = 0}^{M - k - 2} a_n a_{n + k + 1} + C \sum_{n = M - k -1 }^{M-1} \frac{\abs{a_n}}{(k+2+n)^s} \interval{-1, 1}
    ...
    return res + IS_minus_galerkin(k, pb);
}

template <int m, int M>
capd::pdes::PolynomialBound<capd::interval, unsigned, M> KSDissipativeOperator<m, M>::guess_far_tail(const capd::IVector &x, const capd::pdes::PolynomialBound<capd::interval, unsigned, M> &T0) const{
    capd::pdes::PolynomialBound<capd::interval, unsigned, M> res;
...
template <int m, int M>
capd::interval KSDissipativeOperator<m, M>::projection_error(int k, const capd::pdes::PolynomialBound<capd::interval, unsigned, M> &pb) const{
